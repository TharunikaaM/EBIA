...
class IdeaService:
    ...
    def save_evaluation(self, db: Session, email: str, idea: str, results: IdeaResponse):
        """
        Saves the evaluation result to the database.
        """
        try:
            # Convert IdeaResponse to a dict that is JSON serializable
            ...
            db.add(new_entry)
            db.commit()
            logger.info(f"Saved evaluation history for {email}")
        except Exception as e:
